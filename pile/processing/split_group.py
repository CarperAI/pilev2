...
meta_match = {
    "arXiv_ver2": lambda example: ast.literal_eval(example["meta"]["source"])
    == "arXiv_out/",
    "PubMed_ver2": lambda example: ast.literal_eval(example["meta"]["source"])
    == "PubMedDataset",
    "Gutenberg_ver2": lambda example: ast.literal_eval(example["meta"]["source"])
    == "Project Gutenberg",
    "FreeLaw_Options_ver2": lambda example: "date_created" in example.keys(),
    "UbuntuIRC_ver2": lambda example: ast.literal_eval(example["meta"]["source"])
    == "Ubuntu IRC",
    ...
    "USPTO_ver2": lambda example: ast.literal_eval(example["meta"]["source_data"])
    == "USPTO-Application",
    "PileOfLaw_ver2": lambda example: "dataset" in example.keys(),
    "OtherWiki_ver2": lambda example: "wiki_source" in example.keys(),
    "S2ORC_ver2": lambda example: ast.literal_eval(example["meta"]["source"])
    == "S2ORC",
}
...
if __name__ == "__main__":
    ...
    for subset_key in meta_match.keys():
        ...
        subset_dataset = dataset.filter(
            meta_match[subset_key], num_proc=args.num_workers
        )
