...
def subset_dataset_fn(dataset, percentage: float = 10.0):
    if len(dataset) > 10:
        ...
        print(
            f"Slicing 10% which is {percentage_total} |  {len(dataset)} out of {total_len}"
        )
        logger.info(
            f"Slicing 10% which is {percentage_total} | {len(dataset)} out of {total_len}"
        )
    ...
    for sub_dir in tqdm(input_sub_dirs, leave=False):
        try:
            if sub_dir != "non_local_dedup":
                ...
                logger.info("The subdir is non_local_dedup")
                ...
                for non_local_sub_dir in tqdm(non_local_sub_dirs, leave=False):
                    path_non_local_sub_dir = os.path.join(
                        args.input_dir, sub_dir, non_local_sub_dir
                    )
                    print(path_sub_dir)
                    logger.info(path_sub_dir)
                    ...
                    output_dir = pathlib.Path(
                        os.path.join(args.output_dir, sub_dir, non_local_sub_dir)
                    )
                    output_dir.mkdir(parents=True, exist_ok=True)
                    subset_dataset.save_to_disk(args.output_dir)
                    write_to_json(output_dir, subset_idx)
